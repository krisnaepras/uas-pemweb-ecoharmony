...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white shadow-md rounded p-4">
            ...
            <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden border border-gray-300">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider">Nama
                            Produk</th>
                    ...
                    @foreach ($products as $product)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a href="javascript:void(0)" class="text-gray-900"
                                    onclick="toggleDetails({{ $product->id }})">
                                    {{ $product->nama_produk }}
                                </a>
                                <div id="productDetails{{ $product->id }}"
                                    class="mt-2 p-4 bg-gray-100 rounded shadow-inner" style="display:none;">
                                    ...
                                    <img src="{{ asset('images/' . $product->gambar_produk) }}"
                                        alt="{{ $product->nama_produk }}" class="w-24">

                                </div>
                                <div id="editProduct{{ $product->id }}" class="mt-2 p-4 bg-gray-100 rounded shadow-inner"
                                    style="display:none;">
                                    <form action="{{ route('admin.products.update', $product->id) }}" method="POST"
                                        ...
                                        <div class="mb-4">
                                            <label for="harga_produk" class="block text-sm font-medium text-gray-700">Harga
                                                Produk</label>
                                        ...
                                        <div class="mb-4">
                                            <label for="stok_produk" class="block text-sm font-medium text-gray-700">Stok
                                                Produk</label>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <input type="file" name="gambar_produk"
                                                class="mt-1 block w-full border border-gray-300 rounded-md p-2">
                                            <img src="{{ asset('images/' . $product->gambar_produk) }}"
                                                alt="{{ $product->nama_produk }}" class="w-24 mt-2">
                                        </div>
                                        <div class="mb-4">
                                            <label for="deskripsi_produk"
                                                class="block text-sm font-medium text-gray-700">Deskripsi Produk</label>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="inline-flex space-x-2">
                                    <button class="bg-yellow-500 text-white px-4 py-2 rounded"
                                        onclick="toggleEdit({{ $product->id }})">Edit</button>
                                    <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST"
                                        class="inline bg-red-600 px-4 py-2 rounded">
                                        @csrf
                                        @method('DELETE')
        ...
        <button class="bg-blue-700 mt-2 text-white px-4 py-2 rounded" onclick="toggleForm()">Tambah Produk</button>
        <div id="addProductForm" class="mt-4 p-4 bg-white rounded shadow-md" style="display:none;">
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    <label for="nama_produk" class="block text-sm font-medium text-gray-700">Nama Produk</label>
                    <input type="text" name="nama_produk" class="mt-1 block w-full border border-gray-300 rounded-md p-2"
                        required>
                </div>
                <div class="mb-4">
